  ...
  return (
    <div class="feeds-page container">
      <div class="scrollable">
        ...
        >
          <input type="text" placeholder="URL" value={linkToCreate()} onInput={(e) => setLinkToCreate(e.currentTarget.value)} />
          <button type="submit">Add & Subscribe</button>
          <span class="row">
            <input
              type="checkbox"
              id="fetch_old_items"
              name="fetch_old_items"
              checked={fetchOldItems()}
              onChange={(e) => enableFetchOldItems(e.currentTarget.checked)}
            />
            <label for="fetch_old_items">
              <small>Fetch old items</small>
            </label>
          </span>
        </form>
        <ul class="feed-list">
              ...
              <li class={`${feed.status == feedApi.FeedStatus.UNSUBSCRIBED ? "lowp" : ""}`}>
                <div class="row">
                  <Switch>
                    ...
                    <Match when={feed.id === idToUpdate()}>
                      <input type="text" value={feed.title} onInput={(e) => setTitleToUpdate(e.currentTarget.value)} />
                      <input type="text" value={feed.link} onInput={(e) => setLinkToUpdate(e.currentTarget.value)} />
                    </Match>
                  </Switch>
                ...
                <small>
                  ...
                  <span title={dayjs(feed.checked_at).tz(dayjs.tz.guess()).format()}>{dayjs(feed.checked_at).fromNow()}</span>
                  <span class="sep"> | </span>
                  <Switch>
                    <Match when={feed.id !== idToUpdate()}>
